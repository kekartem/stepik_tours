...
class MainView(View):
    def get(self, request):
        ...
        for i in range(6):
            ...
            if tours[j] not in randomTours.items():
                randomTours[i + 1] = tours[j]
                randomTours[i + 1]['id'] = j
        data = {"tours": randomTours}
        print(data)
    ...
    def get(self, request, departure):
        ...
        for key, value in tours.items():
            if value['departure'] == departure:
                value['id'] = key
                depTours.append(value)
                if value['price'] < minPrice:
                    minPrice = value['price']
                if value['price'] > maxPrice:
                    maxPrice = value['price']
                if value['nights'] < minNights:
                    minNights = value['nights']
                if value['nights'] > maxNights:
                    maxNights = value['nights']

        data = {"tours": depTours, "from": departures[departure], "amount": len(depTours), "minPrice": minPrice,
                "maxPrice": maxPrice, "minNights": minNights, "maxNights": maxNights}
...
class TourView(View):
    def get(self, request, tour_id):
        tour = tours[tour_id]
        if tour['departure'] in departures:
            tour['departure'] = departures[tour['departure']]
        data = {"tour": tours[tour_id], "stars": range(int(tour['stars'])), "starsLeft": range(5 - int(tour['stars']))}
        print(range(int(tour['stars'])))
